...
function staggerBedTransfers(bedTransfers: number[]): number[][] {
  let transfers: number[][] = [];
  let FTBEven: number[] | undefined, FTBOdd: number[] | undefined;
  let BTFEven: number[] | undefined, BTFOdd: number[] | undefined;
...
function resolveOffsets(pass: number[], offsets: number[]): number[][] {
  let bedTransfers = new Array(pass.length).fill(0);

  let seenOffsets = new Set<number>();
  ...
  for (const currentRacking of rackings) {
    ...
    for (let i = 0; i < offsets.length; i++) {
      if (offsets[i] == currentRacking) {
        const rackingEntry = RACKINGS.at(currentRacking);
        if (!rackingEntry) continue;
        if (pass[i] == stitches.KNIT) {
          transfersAtRacking[i] = rackingEntry[0];
        } else if (pass[i] == stitches.PURL) {
          transfersAtRacking[i] = rackingEntry[1];
        }
      }
    }
...
export function scheduleChart(machineChart: Bimp, yarnSequence: number[]) {
