...
const hotkeys: Record<string, () => void> = {
  ...
  ...Object.fromEntries(
    Object.entries(toolData).map(([toolId, td]) => [
      td.hotkey,
      () => dispatch({ activeBlockTool: toolId }),
    ])
  ),
...
function numberPressed(num: number) {
  // number key was pressed
  console.debug(`${num} key pressed`);
}

export function globalKeydown(e: KeyboardEvent) {
...
export function globalKeyup(e: KeyboardEvent) {
  const newHeldKeys = new Set(GLOBAL_STATE.heldKeys);
