...
export function removeBlock(blockIndex: number) {
...
export function resizeBlock(e: PointerEvent, blockIndex: number, direction: string) {
  const { blocks, blockEditMode, activeSymbol, activeYarn } = GLOBAL_STATE;

  const blockType = blockEditMode == "stitch" ? "stitchBlock" as const : "yarnBlock" as const;
  ...
  function move(e: PointerEvent) {
    if (e.buttons == 0) {
      ...
      let updatedBlocks = blocks.map(b => ({ ...b })) as Block[];
...
export function moveBlock(e: PointerEvent, blockIndex: number) {
  ...
  function move(e: PointerEvent) {
    if (e.buttons == 0) {
      ...
      let updated = blocks.map(b => ({ ...b })) as Block[];
      updated[blockIndex].pos = [x - dx, y + dy] as Vec2;
...
export function editBlock(e: PointerEvent, blockIndex: number) {
  if (e.which == 2) {
    pan(e);
    return;
  }
  ...
  let init = blocks.map(b => ({ ...b })) as Block[];
