...
function punch(x: number, y: number, r: number = machineConfigs[gc.machine as string].punchRadius) {
  const points = [];

  const angle = (2 * Math.PI) / gc.numSides;

  for (let i = 0; i < gc.numSides; i++) {
...
function overlap() {
  const { b, c, h, v, cols } = machineConfigs[gc.machine as string];
  const punches = [];
  for (let i = 0; i < cols; i++) {
    punches.push(punch(b + h / 2 + i * h, c + v / 2 - v));
    punches.push(punch(b + h / 2 + i * h, c + v / 2 - 2 * v));

    punches.push(punch(b + h / 2 + i * h, c + v / 2 + gc.rows * v));
    punches.push(
      punch(b + h / 2 + i * h, c + v / 2 + (gc.rows + 1) * v)
    );
  }
...
function pattern() {
  const { b, h, c, v, cols } = machineConfigs[gc.machine as string];
...
function belt() {
  const { b, h, c, v, cols, punchRadius } =
    machineConfigs[gc.machine as string];
  const punches = [];

  for (let j = -2; j < gc.rows + 2; j++) {
...
function loop() {
  const { b, c, v, h, cols } = machineConfigs[gc.machine as string];
  const punches = [];

  for (const i of [-1, 0, gc.rows / 2 - 1, gc.rows / 2]) {
...
function cardHeight() {
  const { v, c } = machineConfigs[gc.machine as string];
  return gc.rows * v + 2 * c;
}

function cardWidth() {
  const { cols, h, b } = machineConfigs[gc.machine as string];
  return cols * h + 2 * b;
}
