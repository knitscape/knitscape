...
export function currentTargetPointerPos(e: PointerEvent): [number, number] {
  const rect = (e.currentTarget as Element).getBoundingClientRect();
  return [e.clientX - rect.left, rect.height - (e.clientY - rect.top)];
}

export function pointerPosInElement(e: PointerEvent, el: Element): [number, number] {
...
export function boundaryBbox(boundary: [number, number][]) {
  let xMin = Infinity;
  let yMin = Infinity;
  let xMax = -Infinity;
  let yMax = -Infinity;
