...
export const gridPattern = (cellWidth: number, cellHeight: number) => svg`
<pattern
  id="grid"
  width="${cellWidth}"
  height="${cellHeight}"
...
export const maskOutside = (boundary: [number, number][] | null, chart: { width: number; height: number } | null, cellWidth: number, cellHeight: number) => {
  if (!boundary || !chart) return;
  return svg`

<mask id="mask">
  <rect x="0" y="0" width="100%" height="100%" fill="#ffffffff" />
  <path d="M ${boundary.reduce(
    (acc: string, [x, y]: [number, number]) => `${acc} ${x * cellWidth} ${y * cellHeight}`,
    ""
  )} Z" />
</mask>
<filter id="g">
