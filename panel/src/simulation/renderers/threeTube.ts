...
let renderer: any, camera: any, scene: any, controls: any, effect: any;
let yarns: any[] = [];

function fitCameraToBbox(camera: any, bbox: any, offset: any) {
...
function updateCameraExtents(camera: any, bbox: any, orbitControls: any) {
...
function generateCNPoints(pointArray: any, color: any, pointSize = 0.1) {
...
function init(yarnData: any, canvas: HTMLCanvasElement) {
  let width = (canvas.parentNode as HTMLElement).clientWidth;
  let height = (canvas.parentNode as HTMLElement).clientHeight;
  ...
  yarnData.forEach((yarn: any) => {
    ...
    let color = new THREE.Color(yarn.color[0], yarn.color[1], yarn.color[2]);
    color.setRGB(yarn.color[0], yarn.color[1], yarn.color[2]);
  ...
  if (!camera) {
    ...
    fitCameraToBbox(camera, bbox, 1.2);
  }
...
function updateYarnGeometry(yarnData: any) {
  yarnData.forEach((yarn: any, yarnIndex: any) => {
