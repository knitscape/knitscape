...
function clearLastDrawn(blocks: Block[]): LastDrawnEntry[] {
  return blocks.map((block) => {
    return { bitmap: null, pos: [...block.pos], width: null, height: null };
  });
}

function rescaleAll(blocks: Block[], editMode: string | null, cellWidth: number, cellHeight: number) {
  blocks.forEach((block, blockIndex) => {
    let bitmap = editMode == "stitch" ? block.stitchBlock : block.yarnBlock;
    setCanvasSize(
      document.querySelector(`[data-blockindex="${blockIndex}"]`) as HTMLCanvasElement,
      ...
      syncState(state: GlobalState) {
        ...
        state.blocks.forEach((block, blockIndex) => {
          ...
          if (lastDrawn[blockIndex].bitmap != bitmap) {
            ...
            let offset: [number, number] = [block.pos[0] - bbox.xMin, block.pos[1] - bbox.yMin];

            if (blockEditMode == "stitch" && state.yarnChart && state.chart && state.yarnPalette) {
              drawStitchBlock(
                document.querySelector(`[data-blockindex="${blockIndex}"]`) as HTMLCanvasElement,
                state.colorMode,
                bitmap,
            ...
            } else if (blockEditMode == "yarn" && state.yarnChart && state.yarnPalette) {
              drawYarnBlock(
                document.querySelector(`[data-blockindex="${blockIndex}"]`) as HTMLCanvasElement,
                bitmap,
                offset,
