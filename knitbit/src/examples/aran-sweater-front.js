// An Aran-style cabled sweater front: the shaping from "Sweater Front"
// (rib hem, armholes, split neck, short-row shoulders) knit in a pattern of
// cable panels, mirrored about the center:
//
//   moss | rope | honeycomb | braid | diamond | braid | honeycomb | rope | moss
//
...
const MAIN = "#eee3cb"; // undyed cream wool
const WASTE = "#e07a5f";
...
// 3-over-3 rope, crossed every 8 courses.
function rope() {
  return {
    width: 6,
    stitch: knit,
    crosses: (c) => (c % 8 === 0 ? [{ x: 0, left: 3, right: 3, over: "left" }] : []),
  };
}
...
// Honeycomb: in each 8-stitch repeat, two 2-over-2 crosses whose front
// strands move apart, then four courses later cross back toward each other.
function honeycomb(repeats) {
  return {
    width: 8 * repeats,
    stitch: knit,
    crosses: (c) => {
      ...
      for (let r = 0; r < repeats; r++) {
        out.push({ x: 8 * r, left: 2, right: 2, over: flip ? "left" : "right" });
        out.push({ x: 8 * r + 4, left: 2, right: 2, over: flip ? "right" : "left" });
      }
      return out;
    },
  };
...
// Diamond: two 2-stitch knit strands on a purl background travel apart one
// needle every other course (crossing over the purl stitch beside them),
// then back together, where they cross each other. The inside is moss.
function diamond(reach) {
  const width = 2 * reach + 4;
  const steps = 2 * reach + 1; // one per crossing course
  // How far each strand sits out from the center after step s.
  const offset = (s) => (s <= reach ? s : 2 * reach - s);
  const step = (c) => Math.floor(c / 2) % steps;
  return {
    width,
    stitch: (x, c) => {
      const o = offset(step(c));
      const l = reach - o; // left strand's first needle
      const r = reach + 2 + o; // right strand's first needle
      if (x === l || x === l + 1 || x === r || x === r + 1) return "k";
      if (x > l + 1 && x < r) return mossStitch(x, c);
      return "p";
    },
    crosses: (c) => {
      if (c % 2 !== 0) return [];
      const s = step(c);
      if (s === 0) return [{ x: reach, left: 2, right: 2, over: "left" }];
      const before = offset(s - 1);
      const after = offset(s);
      if (after > before) {
        // Traveling out: each strand crosses over the stitch outside it.
        return [
          { x: reach - after, left: 1, right: 2, over: "right" },
          { x: reach + 2 + before, left: 2, right: 1, over: "left" },
        ];
      }
      // Traveling in: each strand crosses over the stitch inside it.
      return [
        { x: reach - before, left: 2, right: 1, over: "left" },
        { x: reach + 2 + after, left: 1, right: 2, over: "right" },
      ];
    },
  };
}
...
const CENTER = diamond(8);

// Lay the panels out across the needles, filling the sides with moss.
...
function stitchAt(x, c) {
  ...
  return entry.panel.stitch(lx, c + PHASE);
}

// All crosses on body course c, in needle coordinates.
function crossesAt(c) {
  const out = [];
  for (const { panel, start, mirror } of placed) {
    for (const cr of panel.crosses(c + PHASE)) {
      if (!mirror) {
