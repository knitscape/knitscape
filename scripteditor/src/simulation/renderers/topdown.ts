...
let segmentProgramInfo: any, joinProgramInfo: any, camera: Camera2D, bbox: any;

let yarnProgramData: any[] = [];
...
function mouseClip(e: any) {
  ...
  const normalizedX = cssX / domCanvas.clientWidth;
  const normalizedY = cssY / domCanvas.clientHeight;
...
function buildViewProjection() {
  ...
  let projectionMatrix = Mat3.projection(
    domCanvas.clientWidth,
    domCanvas.clientHeight
  );
...
function initializeYarn(yarn: any) {
...
export function init(yarnData: any, canvas: HTMLCanvasElement) {
  ...
  if (!camera) {
    camera = new Camera2D();
    camera.fit(bbox);
    canvas.addEventListener("pointerdown", (e: any) => camera.handlePan(e));
    canvas.addEventListener("wheel", (e: any) => camera.handleZoom(e));
  }
}

function drawMainYarn(uniforms: any, instances: any) {
  gl.uniform3f(
...
function drawJoin(uniforms: any, instances: any) {
