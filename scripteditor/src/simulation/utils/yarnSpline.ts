const Vec2 = {
  add([ax, ay]: number[], [bx, by]: number[]): number[] {
    return [ax + bx, ay + by];
  },

  sub([ax, ay]: number[], [bx, by]: number[]): number[] {
    return [ax - bx, ay - by];
  },

  scale([x, y]: number[], scalar: number): number[] {
    return [x * scalar, y * scalar];
  },

  abs([x, y]: number[]): number[] {
    return [Math.abs(x), Math.abs(y)];
  },

  mag([x, y]: number[]): number {
    return Math.sqrt(x * x + y * y);
  },

  dot([ax, ay]: number[], [bx, by]: number[]): number {
    return ax * bx + ay * by;
  },
...
function calcM(
  ...
  function dist([x1, y1]: number[], [x2, y2]: number[]): number {
...
export function splitYarnSpline(
  p0: number[],
  [p1x, p1y]: number[],
  [p2x, p2y]: number[],
  p3: number[],
