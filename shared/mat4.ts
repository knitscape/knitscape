...
export const Mat4 = {
  ...
  transformVector(m: number[], v: number[]): number[] {
    var dst: number[] = [];
    for (var i = 0; i < 4; ++i) {
      dst[i] = 0.0;
      for (var j = 0; j < 4; ++j) {
  ...
  xRotate(m: number[], angleInRadians: number): number[] {
    return Mat4.multiply(m, Mat4.xRotation(angleInRadians));
  },

  yRotate(m: number[], angleInRadians: number): number[] {
    return Mat4.multiply(m, Mat4.yRotation(angleInRadians));
  },

  zRotate(m: number[], angleInRadians: number): number[] {
    return Mat4.multiply(m, Mat4.zRotation(angleInRadians));
  },

  scale(m: number[], sx: number, sy: number, sz: number): number[] {
