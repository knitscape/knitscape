...
export function simulate(
  ...
  const yarnPalette: Record<string | number, string> = {
  ...
  function layoutNodes(yarnGraph: YarnModel): ContactNode[] {
  ...
  function yarnCurve(yarnLink: YarnPathLink): string {
    const index = yarnLink.index;

    if (index == 0 || index > yarnPathLinks.length - 3) {
      const src = yarnLink.source as ContactNode;
      const tgt = yarnLink.target as ContactNode;
      return `M ${src.x} ${src.y} ${tgt.x} ${tgt.y}`;
    }
  ...
  function sortSegments(): Record<
    string,
    Record<string | number, string[]>
  > {
    const sortedSegments: Record<string, Record<string | number, string[]>> = {
      front: { border: [] },
      back: { border: [] },
      mid: { border: [] },
    };
  ...
  const [backCanvas, midCanvas, frontCanvas] = getCanvases([
    "back",
    "mid",
    "front",
  ]);
