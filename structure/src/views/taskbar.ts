  ...
  return html`<div id="taskbar">
    ...
    <div id="chart-size-controls">
      ...
      <input
        ...
        @change=${(e: Event) =>
          updateChartWidth(Number((e.target as HTMLInputElement).value))}
        min="5"
        max="1000" />
      <label>Height</label>
      <input
        ...
        @change=${(e: Event) =>
          updateChartHeight(Number((e.target as HTMLInputElement).value))}
        min="5"
        max="1000" />
    </div>
    <div class="taskbar-buttons">
      ...
      <button class="btn icon" @click=${() => toggleFullscreen()}>
        <i
          class="fa-solid fa-${!(window.document as any).fullscreenElement &&
